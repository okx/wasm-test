...
#exchaincli query wasm contract-state smart "$contractAddr" '{"balance":{"address":"ex1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq"}}' not support

res=$(exchaincli tx send captain $contractAddr 1okt --fees 0.001okt -y -b block)



addr1=$(exchaincli keys add addr1 -y 2>&1 | jq -r '.address')  #0x
