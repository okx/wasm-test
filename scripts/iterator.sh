...
test_address() {
  cli="exchaincli query wasm contract-state smart  $1 $2"
  res=$($cli)
...
test_case(){
  ...
  res=$(exchaincli tx wasm store $contract_dir/artifacts/iterator.wasm --fees 0.01okt --from user --gas=5000000 -b block -y)
  code_id=$(echo "$res" | jq '.logs[0].events[1].attributes[0].value' | sed 's/\"//g')
  res=$(exchaincli tx wasm instantiate "$code_id" "{}" --label test1 --admin ${useraddr_0x} --fees 0.1okt --gas=5000000 --from user -b block -y)
